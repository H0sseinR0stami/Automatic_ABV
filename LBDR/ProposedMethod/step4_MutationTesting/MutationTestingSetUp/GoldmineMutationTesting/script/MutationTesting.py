...
# make sure that "rtl" folder does contain "LBDR.sv" file
if os.path.exists("../rtl/LBDR.sv"):
    os.remove("../rtl/LBDR.sv")
...
# read mutant file's names one by one  from 'mutant_files_name.txt' and move it to rtl foldr and rename it to "LBDR.sv"
files_names_list: TextIO = open(r"../script/mutant_files_name.txt", 'r')
for file in files_names_list:
    os.rename(os.path.join(mutants_path, file.rstrip('\n')), os.path.join(rtl_path, 'LBDR.sv'))
    ...
    # move "LBDR.sv" from "rtl" folder and rename it to its real name and save it to "simulated_mutants" folder
    # remove "mutant"
    os.rename(os.path.join(rtl_path, "LBDR.sv"),
    ...
    try:
        os.rename(os.path.join(sim_path, "LBDR_assertion_report.xml"),
                  os.path.join(simulation_results_assertion_path,
                               file.rstrip('.sv\n') + "_LBDR_assertion_report.xml"))

    # os.rename(os.path.join(sim_path, "Arbiter_cover_report_req_grant_check.xml"), os.path.join(
    # simulation_results_cover_path, file.rstrip('.sv\n') + "_Arbiter_cover_report_req_grant_check.xml"))
    except:
        print("error")
...
for file in files_names_list:
    with open(os.path.join(simulation_results_assertion_path_csv, file.rstrip('\n')), 'r') as csvinput:
        with open(os.path.join(simulation_results_assertion_path_csv_final,
                               file.replace("mutant", "").rstrip('LBDR_assertion_report.csv\n'))
                  + ".csv", 'w') as csvoutput:
