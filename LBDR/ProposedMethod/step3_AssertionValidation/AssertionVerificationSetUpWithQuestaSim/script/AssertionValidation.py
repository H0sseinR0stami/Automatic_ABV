...
'''
This script simulate the LBDR model and comment the Assertions which fail at the simulation
...
# run "sim_LBDR.do" to simulate "LBDR.sv"
os.chdir(sim_path)
os.system('cmd /c "vsim -do sim_LBDR.do"')
...
try:
    os.rename(os.path.join(sim_path, "LBDR_assertion_report.xml"),
              os.path.join(simulation_results_assertion_path, "LBDR_assertion_report.xml"))
except:
    print("error")
...
# xml to csv
tree = et.parse(os.path.join(simulation_results_assertion_path, "LBDR_assertion_report.xml"))
with open(os.path.join(simulation_results_assertion_path_csv, "LBDR_assertion_report.csv"),
...
with open(os.path.join(simulation_results_assertion_path_csv, "LBDR_assertion_report.csv"), 'r') as csvinput:
    with open(os.path.join(properties_sva_path, "failed_LBDR_assertion_report.txt"),
              'w') as csvoutput:
...
# list and save the Assertions which are failed in "failed_LBDR_assertion_report.txt" file located in  "../properties_sva" folder
os.chdir(properties_sva_path)
with open("failed_LBDR_assertion_report.txt","r") as f, open("failed_assertions.txt","w") as outfile:
 for i in f.readlines():
       if not i.strip():
           continue
       if i:
           outfile.write(i)
os.remove("failed_LBDR_assertion_report.txt")

# comment the failed Assertions in "LBDR_properties.sva" file
os.chdir('../properties_sva')
with open("failed_assertions.txt", "r") as f:
    for number in f.readlines():
        add_text_to_beginning_of_the_line(LBDR_properties_sva, 'property_' + str(number.strip('\n')) + '_assert', "//", 0)
if os.path.exists("../simulation_results"):
    shutil.rmtree("../simulation_results")
