# Hossein Rostami (Rostami.Hossein@znu.ac.ir)
'''
This script simulate the Arbiter Design and comment the Assertions which fail at the simulation
...
os.system('cmd /c "vsim -do sim_Arbiter.do"')
...
try:
    os.rename(os.path.join(sim_path, "Arbiter_assertion_report.xml"),
              os.path.join(simulation_results_assertion_path, "Arbiter_assertion_report.xml"))
...
# xml to csv
tree = et.parse(os.path.join(simulation_results_assertion_path, "Arbiter_assertion_report.xml"))
with open(os.path.join(simulation_results_assertion_path_csv, "Arbiter_assertion_report.csv"),
...
with open(os.path.join(simulation_results_assertion_path_csv, "Arbiter_assertion_report.csv"), 'r') as csvinput:
    with open(os.path.join(properties_sva_path, "failed_Arbiter_assertion_report.txt"),
              'w') as csvoutput:
...
# list and save the Assertions which are failed in "failed_Arbiter_assertion_report.txt" file located in  "../properties_sva" folder
os.chdir(properties_sva_path)
with open("failed_Arbiter_assertion_report.txt","r") as f, open("failed_assertions.txt","w") as outfile:
...
os.remove("failed_Arbiter_assertion_report.txt")

# comment the failed Assertions in "Arbiter_properties.sva" file
os.chdir('../properties_sva')
with open("failed_assertions.txt", "r") as f:
    for number in f.readlines():
        add_text_to_beginning_of_the_line(Arbiter_properties_sva, 'property_' + str(number.strip('\n')) + '_assert', "//", 0)
