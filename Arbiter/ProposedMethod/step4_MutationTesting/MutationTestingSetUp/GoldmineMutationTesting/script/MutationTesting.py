...
'''
script for doing mutation testing
...
if os.path.exists("../rtl/Arbiter.sv"):
    os.remove("../rtl/Arbiter.sv")
...
for file in files_names_list:
    ...
    # remove "mutant"
    os.rename(os.path.join(rtl_path, "Arbiter.sv"),
...
for file in files_names_list:
    with open(os.path.join(simulation_results_assertion_path_csv, file.rstrip('\n')), 'r') as csvinput:
        with open(os.path.join(simulation_results_assertion_path_csv_final,
                               file.replace("mutant", "").rstrip('Arbiter_assertion_report.csv\n'))
                  + ".csv", 'w') as csvoutput:
